...
class Conv(object):
  ...
  def backward(dout, cache):
    ...
    x_pad = torch.nn.functional.pad(x, (p,p,p,p))
    ...
    for n in range(N):
      for f in range(F):
        for i in range(H_out):
          for j in range(W_out):
            dw[f,:,:,:] += dout[n,f,i,j] * x_pad[n,:,s*i:s*i+HH, s*j:s*j+WW]
            db[f] += dout[n,f,i,j]
            dx[n,:,s*i:s*i+HH, s*j:s*j+WW] += dout[n,f,i,j] * w[f,:,:,:]
  ...
  def forward(x, pool_param):
    ...
    for n in range(N):
      for c in range(C):
        for i in range(H_out):
          for j in range(W_out):
            out[n,c,i,j] = torch.max(x[n,c, s*i:s*i+HH, s*j:s*j+WW])
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    cache = (x, pool_param)
  ...
  def backward(dout, cache):
    ...
    for n in range(N):
      for c in range(C):
        for i in range(H_out):
          for j in range(W_out):
            max_val = torch.max(x[n,c, s*i:s*i+HH, s*j:s*j+WW])
            dx[n,c,s*i:s*i+HH, s*j:s*j+WW]+=dout[n,c,i,j]*(max_val==x[n,c,s*i:s*i+HH, s*j:s*j+WW])
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    return dx
...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    #spatial dimensions preserved after convolution
    #spatial dimensions reduced by a factor of 2 after max pooling
    in_linear_dim = num_filters*(H//2)*(W//2) #length of falttened tensor
    self.params['W2'] = weight_scale * torch.randn(in_linear_dim, hidden_dim, dtype=dtype, device=device)
...
class DeepConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32),
    ...
    C,H,W = input_dims
    C_list = [C]
    C_list = C_list+num_filters

    if weight_scale == "kaiming":
      for i in range(self.num_layers-1):
        self.params[f'W{i+1}'] = kaiming_initializer(C_list[i], num_filters[i], K=3, relu=True, device=device,dtype=dtype)
      ...
      final_in_dim = num_filters[-1]*(W//(2**num_max_pool))*(H//(2**num_max_pool))
      self.params[f'W{self.num_layers}'] = kaiming_initializer(final_in_dim, num_classes, K=None, relu=False, device=device,dtype=dtype)
      self.params[f'b{self.num_layers}'] = torch.zeros(num_classes, dtype=dtype, device=device)

    else:
      for i in range(self.num_layers-1):
        self.params[f'W{i+1}'] = weight_scale * torch.randn(num_filters[i],C_list[i],3,3, dtype=dtype, device=device)
        self.params[f'b{i+1}'] = torch.zeros(num_filters[i], dtype=dtype, device=device)
        if(batchnorm==True):
            self.params[f'gamma{i+1}'] = torch.ones(num_filters[i], dtype=dtype, device=device)
            self.params[f'beta{i+1}'] = torch.zeros(num_filters[i], dtype=dtype, device=device)
      ...
      num_max_pool = len(max_pools)
      final_in_dim = num_filters[-1]*(W//(2**num_max_pool))*(H//(2**num_max_pool))
      self.params[f'W{self.num_layers}'] = weight_scale * torch.randn(final_in_dim,num_classes, dtype=dtype, device=device)
      self.params[f'b{self.num_layers}'] = torch.zeros(num_classes, dtype=dtype, device=device)
  ...
  def loss(self, X, y=None):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    identity_pool_param = {'pool_height': 1, 'pool_width': 1, 'stride': 1}
    max_pool_set = {i for i in self.max_pools}
    ...
    if self.batchnorm:
      for i in range(self.num_layers-1):
        ...
        if (i in max_pool_set):
          out, cache = Conv_BatchNorm_ReLU_Pool.forward(out,W,b,gamma,beta,conv_param,self.bn_params[i],pool_param)
        else:
          out, cache = Conv_BatchNorm_ReLU_Pool.forward(out,W,b,gamma,beta,conv_param,self.bn_params[i],identity_pool_param)
        caches.append(cache)
    else:
      for i in range(self.num_layers-1):
        W = self.params[f'W{i+1}']
        ...
        if (i in max_pool_set):
          ...
          out, cache = Conv_ReLU_Pool.forward(out,W,b,conv_param,identity_pool_param)
        caches.append(cache)
    
    
    out, cache = Linear.forward(out,self.params[f'W{self.num_layers}'],self.params[f'b{self.num_layers}'])
    scores = out
    caches.append(cache)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ...
    dout, dW, db = Linear.backward(dscores,caches[self.num_layers-1])
...
def create_convolutional_solver_instance(data_dict, dtype, device):
  ...
  model = DeepConvNet(num_classes=10,
                    ...
                    max_pools=[0, 1],
                    reg=1e-4, weight_scale='kaiming', dtype=dtype, device=device)
  solver = Solver(model, data_dict,
                  print_every=100, num_epochs=30, batch_size=128,
                  update_rule=adam,
                  optim_config={
                    'learning_rate': 2e-3,
                  },
...
class BatchNorm(object):
  ...
  def forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
      ...
      cache = (x,x_norm,mean,var,gamma,beta,eps,mode)
      #######################################################################
      #                           END OF YOUR CODE                          #
      #######################################################################
    elif mode == 'test':
      ...
      out = gamma*(x_norm) + beta
      cache = (x,x_norm,running_mean,running_var,gamma,eps,beta,mode)
      #######################################################################
      #                           END OF YOUR CODE                          #
      #######################################################################
    else:
  ...
  def backward(dout, cache):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    x,x_norm,mean,var,gamma,beta,eps,mode = cache
    ...
    if mode=='train':
      ...
      dstd = torch.sum(dout*gamma*(-(x-mean)/std**2), dim=0)
      dvar = dstd*(0.5/std)
      dmean = torch.sum(dout*gamma/(-std), dim=0) + dvar*(-2/N*torch.sum(x-mean, dim=0))

      dx = dmean/N + dvar*2*(x-mean)/N + dout*gamma/std

    elif mode =='test':
      dgamma = torch.sum(dout*x_norm, dim=0)
      dbeta = torch.sum(dout,dim=0)
      dx = dout*gamma/(var+eps)**0.5
    else:
      raise ValueError('Invalid forward batchnorm mode "%s"' % mode)
    ###########################################################################
    #                             END OF YOUR CODE                            #
  ...
  def backward_alt(dout, cache):
    ...
    if mode=='train':
      ...
      std = (var+eps)**0.5
      # dvar_dx = 2/N*(x-mean)
      # dx = torch.sum(dout*gamma*(-1/N/std - 0.5*(x-mean)/(std**3)*dvar_dx),dim=0)+dout*gamma/std
      dx = 1/N/std*(N*dout*gamma - torch.sum(dout*gamma,dim=0) - x_norm*torch.sum(dout*gamma*x_norm,dim=0))
