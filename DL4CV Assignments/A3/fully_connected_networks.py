...
class Linear(object):
  ...
  def forward(x, w, b):
    ...
    flatten_x = torch.reshape(x, (x.shape[0], -1))
    out = flatten_x.mm(w) + b
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    cache = (x, w, b)
...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    dout1, dW2, db2 = Linear.backward(dout2, cache2)
    _, dW1, db1 = Linear_ReLU.backward(dout1, cache1)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i in range(len(hidden_dims)-1):
      self.params[f'W{i+2}'] = weight_scale * torch.randn(hidden_dims[i], hidden_dims[i+1], dtype=dtype, device=device)
      self.params[f'b{i+2}'] = torch.zeros(hidden_dims[i+1], dtype=dtype, device=device)
  ...
  def loss(self, X, y=None):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    # initially out is the raw input
    out = X
    caches = []
    drop_caches = []
    for i in range(1, self.num_layers):
      ...
      caches.append(cache)
      if self.use_dropout:
        out, drop_cache = Dropout.forward(out, self.dropout_param)
    ...
    ############################################################################
    # Replace "pass" statement with your code
    #backward for output layer and compute loss
    loss, dout = softmax_loss(scores, y)
    for i in range(self.num_layers):
      loss+=0.5*self.reg*torch.sum(self.params[f"W{i+1}"]**2)
    
    #backward for last hidden layer (not output layer)
    ...
    #backward for other hidden layers
...
def create_solver_instance(data_dict, dtype, device):
  ...
  solver = None
  # Replace "pass" statement with your code
  solver = Solver(model, data_dict, optim_config={"learning_rate": 1e-1}, lr_decay=0.95, num_epochs=20, device="cuda")
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return solver
...
def rmsprop(w, dw, config=None):
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  prev_grad_squared = config["cache"]
  grad_squared = config["decay_rate"]* prev_grad_squared + (1-config["decay_rate"])*dw*dw
...
def adam(w, dw, config=None):
  ...
  moment1_unbias = moment1/(1-B1**config["t"])
  moment2_unbias = moment2/(1-B2**config["t"])
  config['m'] = moment1
  config['v'] = moment2
  next_w = w-config["learning_rate"]*moment1_unbias/(torch.sqrt(moment2_unbias)+config["epsilon"])
  #############################################################################
  #                              END OF YOUR CODE                             #
  #############################################################################
