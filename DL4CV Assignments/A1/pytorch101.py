...
def multiples_of_ten(start, stop):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  nums = torch.arange(start, stop+1)
  x=[]
  for num in nums:
    if num%10==0:
      x.append(num)
...
def slice_assignment_practice(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  replacement = torch.tensor([[0, 1, 2, 2, 2, 2],
  [0, 1, 2, 2, 2, 2],
  [3, 4, 3, 4, 5, 5],
  [3, 4, 3, 4, 5, 5]])

  x[0:4, 0:6] = replacement
...
def reverse_rows(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  idx = torch.tensor([i for i in range(x.shape[0]-1, -1, -1)])
  y = x[idx, :]
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  if use_loop:
    ...
    for i in range(len(x)):
      products.append(torch.mm(x[i], y[i]))
    z = torch.stack(tuple(product for product in products), 0)
...
def normalize_columns(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  means = torch.sum(x, 0)/x.shape[0]
  y = x-means
  stds = torch.sqrt(torch.sum(y*y, 0)/(x.shape[0]-1))
  y = y / stds
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
