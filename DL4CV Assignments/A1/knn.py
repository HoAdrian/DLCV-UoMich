...
def compute_distances_two_loops(x_train, x_test):
  ...
  for i in range(len(x_train)):
    for j in range(len(x_test)):
      i_train = x_train[i]
      j_test = x_test[j]
      dists[i, j] = torch.sum(torch.square(i_train-j_test))
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_one_loop(x_train, x_test):
  ...
  for i in range(len(x_train)):
    ...
    # the sum of squared diff is 1D since dimension is reduced
    dists[i, :] = torch.sum(torch.square(diff), 1)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_no_loops(x_train, x_test):
  ...
  # each entry is the squared magniturde of a vector
  train_squared = torch.sum(torch.square(x_train), 1)
  test_squared = torch.sum(torch.square(x_test), 1)
  # compute ||v-u|| = sqrt(||v||^2-||u||^2-2u.v) for each pair of u and v
  # USE RESHAPE INSTEAD OF TRANSPOSE ON TRAIN_SQUARED
  dists = torch.mm(x_train, x_test.t())*(-2) + test_squared + train_squared.reshape(-1,1)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  for i in range(num_test):
    dists_list = dists[:, i].reshape(num_train)
    min_dists, min_indices = torch.topk(dists_list, k,largest=False)
    k_nearest_labels = [y_train[idx] for idx in min_indices]
    # find most frequent k nearest label
    # mapping k nearest labels to its frequencies
    votes = {}
    for label in k_nearest_labels:
      votes[label.item()] = votes.get(label.item(), 0)+1
    vote_freq = 0
    current_vote = 0
    for label_object in k_nearest_labels:
      label = label_object.item()
      if votes[label]==vote_freq:
        if label<current_vote:
          current_vote=label
      if votes[label]>vote_freq:
        vote_freq = votes[label]
        current_vote = label
    # update predicted label of the ith test example
    y_pred[i] = current_vote
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return y_pred
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
    for i in range(num_folds):
      # use the ith fold as the validation set, other folds as training sets
      x_train_tuple = tuple(x_train_folds[j] for j in range(num_folds) if j!=i)
      y_train_tuple = tuple(y_train_folds[j] for j in range(num_folds) if j!=i)
      x_train_set = torch.concat(x_train_tuple, 0)
      y_train_set = torch.concat(y_train_tuple, 0)
      classifier = KnnClassifier(x_train_set, y_train_set)
      k_to_accuracies[k] = k_to_accuracies.get(k, [])
...
def knn_get_best_k(k_to_accuracies):
  ...
  for k in k_to_accuracies:
    k_accuracy = sum(k_to_accuracies[k])/len(k_to_accuracies[k])
