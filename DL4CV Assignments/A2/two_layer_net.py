...
def nn_forward_pass(params, X):
    ...
    ############################################################################
    Z1 = X.mm(W1)+b1
    hidden = torch.maximum(Z1, torch.zeros_like(Z1))
...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    scores += log_c
    #scores becomes exp(scores)
    scores = torch.exp(scores)
    correct_probs = scores[torch.arange(N), y]/torch.sum(scores, 1) #(N,)
    ...
    # denote scores as Z2, output of relu as h1 and output of first FC layer as Z1
    #calculate dZ2
    dZ2 = torch.zeros_like(scores) #(N,C)
    exp_correct_scores = scores[torch.arange(N), y]
    dZ2 = scores*((correct_probs/exp_correct_scores).reshape(-1,1))
    dZ2[torch.arange(N), y] = correct_probs-1
    #calculate dW2 and db2
    dW2 = h1.t().mm(dZ2)/N #(H,C)
    dW2+=2*reg*W2
    db2 = torch.sum(dZ2, dim=0) / N #(C,)
    #calculate dh1 and dZ1 for backprop
    dh1 = dZ2.mm(W2.t()) #(N,H)
    dZ1 = dh1
    dZ1[h1==0] = 0.  
    #calculate dW1 and db1
    dW1 = X.t().mm(dZ1)/N #(D,H)
    dW1+=2*reg*W1
    db1 = torch.sum(dZ1, dim=0)/N #(H,)
...
def nn_predict(params, loss_func, X):
  ...
  ###########################################################################
  scores = loss_func(params, X, y=None, reg=0)
  values, classes = torch.max(scores, dim=1)
  y_pred = classes
  ###########################################################################
  #                              END OF YOUR CODE                           #
  ###########################################################################
...
def nn_get_search_params():
  ...
  hidden_sizes = [550, 500]
  regularization_strengths = [1e-5, 7e-5]
  learning_rate_decays = [0.95, 5e-1]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
...
def find_best_net(data_dict, get_param_set_fn):
  ...
  # check num models
  if num_models >= 256:
    raise Exception("Please do not test/submit more than 256 items at once")
  elif num_models < 16:
    raise Exception("Please present at least 16 parameter sets in your final ipynb")
    ...
    for hs in hidden_sizes:
      for reg in regularization_strengths:
        for lrd in learning_rate_decays:
          net = TwoLayerNet(input_size, hs, output_size, dtype=torch.float32, device='cuda', std=1e-4)
          stat = net.train(X_train, y_train, X_val, y_val, lr, lrd, reg, num_iters=2000)
          y_val_pred = net.predict(X_val)
          val_acc = (y_val_pred == y_val).float().mean().item()
          #track progress
          i += 1
          print('Training NN %d / %d with learning_rate=%e and hidden_size=%e and reg=%e and decay_rate=%e'% (i, num_models, lr, hs, reg, lrd))
          # update best model
  ...
  #                               END OF YOUR CODE                            #
  #############################################################################
  print(f'best lr={best_lr},hs={best_hs},reg={best_reg},lrd={best_lrd}')
