...
def rnn_backward(dh, cache):
    ...
    N,T,H = dh.shape
    D = cache[0][0].shape[1]
    #each cache in cache is (x,prev_h,Wx,Wh,b,next_h)
    dx = torch.zeros((N,T,D),device = cache[0][0].device, dtype = cache[0][0].dtype)
    dprev_h = torch.zeros_like(cache[0][1])
    dWx = torch.zeros_like(cache[0][2])
    dWh = torch.zeros_like(cache[0][3])
    db = torch.zeros_like(cache[0][4])
    dh0 = torch.zeros_like(dprev_h)

    dprev_h_ = torch.zeros_like(dprev_h)
    for t in range(T-1,-1,-1):
      dx_, dprev_h_, dWx_, dWh_, db_ = rnn_step_backward(dh[:,t,:]+dprev_h_, cache[t])
      dx[:,t,:] = dx_
      dWx+=dWx_
      dWh+=dWh_
      db+=db_
