...
def three_layer_convnet(x, params):
  ...
  a2 = F.relu(z2)
  a2 = flatten(a2, start_dim = 1)
  a3 = F.linear(a2,fc_w,fc_b)
...
def initialize_three_layer_conv_part2(dtype=torch.float, device='cpu'):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  conv_w1 = nn.init.kaiming_normal_(torch.empty(channel_1,C,kernel_size_1,kernel_size_1,dtype=dtype,device=device), nonlinearity='relu')
  conv_b1 = nn.init.zeros_(torch.empty(channel_1, dtype=dtype, device=device))
  conv_w2 = nn.init.kaiming_normal_(torch.empty(channel_2,channel_1,kernel_size_2,kernel_size_2,dtype=dtype,device=device), nonlinearity='relu')
  conv_b2 = nn.init.zeros_(torch.empty(channel_2, dtype=dtype, device=device))
  fc_w = nn.init.kaiming_normal_(torch.empty(num_classes,channel_2*W*H,dtype=dtype,device=device), nonlinearity='relu')
...
class PlainBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    if downsample:
        ...
        nn.BatchNorm2d(Cin),
        nn.ReLU(),
        nn.Conv2d(Cin,Cout,3,padding=1,stride=2),
        nn.BatchNorm2d(Cout),
        nn.ReLU(),
...
class ResNet(nn.Module):
  ...
  def forward(self, x):
    ...
    # Replace "pass" statement with your code
    conv_out = self.cnn(x)
    pool = nn.AvgPool2d((conv_out.shape[2], conv_out.shape[3]))
...
class ResidualBottleneckBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    if downsample:
        ...
        nn.ReLU(),
        nn.Conv2d(Cout//4,Cout//4,3,padding=1,stride=1),
        nn.BatchNorm2d(Cout//4),
        nn.ReLU(),
